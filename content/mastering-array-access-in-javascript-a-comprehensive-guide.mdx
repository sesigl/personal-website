...
The indexOf method is used to find the index of a specific element in an array. In this code, it is used to find the index of the 'banana' element in the fruits array, which is 1.
...
The `push` method adds one or more elements to the end of an array. In this example, `orange` and `pear` are added to the `fruits` array using `push`, increasing the array length to 4. The new length is then logged to the console using `console.log`.
...
The `map` method creates a new array by calling a function on each element of an original array. In this example, the `numbers` array is mapped to a new array `doubled`, where each element is multiplied by 2 using an arrow function. The resulting `doubled` array is then logged to the console.
