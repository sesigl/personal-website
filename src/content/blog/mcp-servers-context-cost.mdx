...
This is the question worth asking for each server in your config: does this tool require MCP, or does my agent already know how to use it from training data and documentation? If it is git, kubectl, the AWS CLI, docker, the agent already knows. CLI beats MCP on context cost every time for these.
...
**The heuristic**: browser control, live research, stateful sessions - use MCP. Git, deployment CLIs, cloud provider APIs - use CLI or skills. When in doubt, try CLI first. You can always add the MCP server later.
...
The mental model that helps: MCP is for things your agent cannot do otherwise. Browser control, live research, stateful external systems, these are the cases where MCP pays for itself. For everything else, your agent already knows the CLI, and CLI does not pollute your context window.

Open your MCP config. For each server, ask: would this workflow be impossible without MCP, or is there a CLI or skill that handles it just as well? Keep the ones that enable real capabilities. Replace the rest with CLI calls or skill documentation. Move what stays into project-scoped `.mcp.json` so each server only loads where it is actually needed.
