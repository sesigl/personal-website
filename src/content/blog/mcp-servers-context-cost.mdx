...
Four servers with 50+ tools consumed 67,000 tokens in my setup. Cloudflare's own MCP best practices [warn against treating an MCP server as a wrapper around your full API schema](https://developers.cloudflare.com/agents/model-context-protocol/), specifically because agents with limited context windows cannot handle it. Their ["Code Mode" research](https://blog.cloudflare.com/code-mode/) found that presenting tools as a TypeScript API instead of raw tool definitions produced dramatically better results, precisely because the tool definition overhead was hurting agent performance.
...
Claude Code also lets you disable globally defined servers at the project level, and scope tools per agent so specific MCP tools only activate for agents that need them.
