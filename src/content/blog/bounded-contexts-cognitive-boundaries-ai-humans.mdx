...
This is not mainly about hitting a hard context limit. Even far below the maximum context window, model performance already goes down as prompts grow larger and noisier. Research shows that irrelevant or badly structured context hurts reasoning quality, even when the model technically "supports" long inputs[^2] [^3].
...
Modern LLMs are built on the Transformer architecture, which relies on attention mechanisms to decide which tokens matter during prediction[^1].
...
This is not just theory. Multiple studies show that even long-context models struggle to reliably use relevant information when the prompt grows, especially when relevant parts are surrounded by distractors or sit somewhere in the middle of a long input[^2] [^3] [^4].
...
xth (what AI generates by default)
...
src/
  ...
  modules/
    pricing/                        (Pricing Bounded Context)
      ...
      domain/                       (PriceEntry, Money, SKU, Promotion…)
      infrastructure/               (InMemoryPriceRepository, WarehouseAdapter…)
    warehouse/                      (Warehouse Bounded Context)
      application/                  (WarehouseService)
      domain/                       (InventoryItem, Quantity, Reservation, SKU…)
      infrastructure/               (InMemoryInventoryRepository…)
  shared/
    contract/
      warehouse/
        AvailabilitySignal.ts       (Published Language)
```

Each bounded context is a vertical slice. It owns its domain, its application logic, and its infrastructure. The only thing that crosses the boundary is the explicit contract: `AvailabilitySignal`.

Notice that both contexts have their own `SKU` value object. This is intentional. An SKU in Pricing is not the same concept as an SKU in Warehouse. They might have the same format today, but their semantics and evolution are independent.
...
It works.
The tests pass.
And you have just created one more coupling making your monolith worse.
...
[^2]: Liu, N., et al. (2023). *Lost in the Middle: How Language Models Use Long Contexts*. arXiv:2307.03172. [https://arxiv.org/abs/2307.03172](https://arxiv.org/abs/2307.03172)

[^3]: Shi, F., et al. (2023). *Large Language Models Can Be Easily Distracted by Irrelevant Context*. arXiv:2302.00093. [https://arxiv.org/abs/2302.00093](https://arxiv.org/abs/2302.00093)

[^4]: Hsieh, C., et al. (2024). *Found in the Middle: How Language Models Use Positional Information*. arXiv:2406.16008. [https://arxiv.org/abs/2406.16008](https://arxiv.org/abs/2406.16008)
